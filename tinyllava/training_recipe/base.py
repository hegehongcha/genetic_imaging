...
class BaseTrainingRecipe:
    ...
    def save(self, model, trainer):
        ...
        # if 'finetune' in self.training_arguments.output_dir: # for finetune stage
        #     if trainer.deepspeed:
        #         torch.cuda.synchronize()
        #     trainer.save_model(self.training_arguments.output_dir)
        #     return
        ...
        # 合并 named_parameters 和 named_buffers
        params = list(model.vision_tower._vision_tower.named_parameters())
        buffers = list(model.vision_tower._vision_tower.named_buffers())
        named_items = params + buffers  # 一起传入

        # 获取完整 state_dict（包含 buffer）
        vision_tower_state_dict = get_state_maybe_zero_3(named_items, [''], require_grad_only=False)
        if trainer.args.local_rank == 0 or trainer.args.local_rank == -1:
