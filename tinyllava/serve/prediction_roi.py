...
model, tokenizer, image_processor, context_len = load_pretrained_model(model_name_or_path="/home/httang/project/llava/TinyLLaVA_Factory-main/gene_checkpoints/llava_factory/tiny-llava-Meta-Llama-3-8B-medroi3d-base-gene-finetune-classification_and_regression/")
model = model.to(device)
text_processor = TextPreprocess(tokenizer, "llama")
...
data_args = model.config
model.to("cuda")
embed_dict = torch.load("/home/httang/project/llava/TinyLLaVA_Factory-main/genetic_dataset/roi_embeds.pt")
# print(model)
with open("/home/httang/project/llava/TinyLLaVA_Factory-main/genetic_dataset/gene_test_classification_and_regression.json", "r") as f:
    samples = json.load(f)
test_examples = []
print("Meta-Llama-3-8B-medroi3d-base-gene-finetune-v2-img-preprocess-5000-frozen-mix-vit-roi-layer-3-randn")
for l, sample in enumerate(samples):
    image_file = sample['image']
    if l%500 == 0:
        print(l)
    idx = image_file.split("/")[0]
    image = embed_dict[idx]
    # img = torch.zeros_like(image)
    img = torch.randn_like(image)
    # image_tensor = image.to("cuda")
    image_tensor = img.to("cuda")
    msg = Message()
...
with open("/home/httang/project/llava/TinyLLaVA_Factory-main/generate/gene_prediction/generated_diseases_Meta-Llama-3-8B-medroi3d-base-gene-finetune-classification_and_regression.json", 'w', encoding='utf-8') as f:
